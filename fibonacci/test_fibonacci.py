...
class FibonacciTest(unittest.TestCase):
    def test_first_few(self):
        """Test the first few elements of the fibonacci sequence, using hard coded values."""
        fibs = fibonacci()
    ...
    def test_one_limit(self):
        """Request a sequence with a limit of one element. Ensure the generator
           is exhausted after reading that element."""
        fibs = fibonacci(1)
        # first next() call ok
        next(fibs)
